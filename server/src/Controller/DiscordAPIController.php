...
class DiscordAPIController extends AbstractController
{
    ...
    public function getUserChannels(Request $request, ServiceRepository $service_repository, UserRepository $user_repository, UserServiceRepository $user_service_repository)
    {
        ...
        // récupération des id des serveurs lié au bot
        ...
        // recup des ids des serveurs de l'utilisateur
        $url = "https://discord.com/api/users/@me/guilds";
        $headers = array(
        ...
        // récupération des IDs des guilds
        $user_guilds = array();
        ...
        // recup du serveur qui est lié au bot et à l'utilisateur
        $common_guilds = array_intersect($user_guilds, $bot_guilds);
        // prendre le 1er et normalement seul serveur lié à l'utilisateur
        $choosen_guild = current($common_guilds);

        // Récupération des informations du serveur (pour le nom du serv)
        $url = "https://discord.com/api/guilds/$choosen_guild";
        $ch = curl_init();
        curl_setopt_array($ch, array(
            CURLOPT_URL => $url,
            CURLOPT_RETURNTRANSFER => true,
        ...
        // Appel de l'API pour récupérer la liste des channels du serveur en question
        $url_channel = "https://discord.com/api/guilds/$choosen_guild/channels";
        $headers = array(
            'Authorization: Bot ' . $bot_token
        );
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => $url_channel,
            CURLOPT_RETURNTRANSFER => true,
        ...
        foreach ($channels as $channel) {
            if ($channel['type'] == 0) { // 0 = channel textuel
                $text_channels[] = array(
                    'id' => $channel['id'],
    ...
    {
        $parameters = array("automation_action_id" => 8);
        $url = "http://localhost/discord/action/check_username/get_parameters";
        if (empty($this->request_api)) {
            $this->request_api = new RequestAPI();
        }
